...
def evaluate_z(z, randomize, reward_type, output_dir, epoch, env_index, single_obj_reward):
    EPSILON = 1e-8
    env = create_env_action(randomize, reward_type, env_index, single_obj_reward)
    env.reset()
    rewards = []
    success = 0
    images = []
    for i in range(24):
        ...
        #z_action = np.clip(z_action, -1 + EPSILON, 1 - EPSILON)
        #print("z_action: ", z_action)
